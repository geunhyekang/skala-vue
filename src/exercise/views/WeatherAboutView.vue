...
<template>
  <v-card variant="outlined">
    <v-card-title>ℹ️ 서비스 소개</v-card-title>
    <v-card-text>
      <p>이 앱은 Vue 3, Vue Router, Pinia, Axios, Vuetify 학습을 위한 날씨 대시보드입니다.</p>
      <ul>
      ...
      <p v-if="usdToKrw" class="text-success mt-3">
        💱 오늘의 환율(기타 외부 API): 1 USD ≈ {{ Math.round(usdToKrw) }} KRW
      </p>
    </v-card-text>
    <v-card-actions>
      <v-btn color="primary" block @click="goToDashboard">메인 대시보드로 이동</v-btn>
    </v-card-actions>
  </v-card>
</template>
